...
for i in range(len(date_list)):
    file_name = 'MiCASA_v1_Rh_x3600_y1800_daily_' + date_list[i] + '.tif'

    
    #tiff_file = file_name
    #file_name = "MiCASA_v1_Rh_x3600_y1800_daily_20230101.tif"
    ...
    with rasterio.open(file_name) as src:
        ...
        cax = ax.imshow(data,extent=[lon.min(), lon.max(), lat.min(), lat.max()], cmap='plasma', vmin=0, vmax=7)
        ...
        # 添加標題
        title = date_list[i] + " "+" Heterotrophic Respiration(Rh) CO2 Flux"
        fig_name = date_list[i] + "Rh CO2 Flux.png"
        #title = "RH"
        #fig_name = 'Rh CO2 Flux.png'
        plt.title(title)
